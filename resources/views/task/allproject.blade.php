...
<div class="table-settings mb-4">
    ...
    @csrf
                ...
                <tr>
                    ...
                    @if(!empty($project->statuses()->first()->name))
                    @if($project->statuses()->first()->name =="В процессе")
                    <td><span class="font-weight-bold text-warning">{{$project->statuses()->first()->name}}</span></td>
                    @elseif($project->statuses()->first()->name =="Готов")
                    <td><span class="font-weight-bold text-success">{{$project->statuses()->first()->name}}</span></td>
                    @elseif($project->statuses()->first()->name =="Невыполненный")
                    <td><span class="font-weight-bold text-danger">{{$project->statuses()->first()->name}}</span></td>
                    @endif
                    @else
                    <td><span class="font-weight-bold text-danger">Null</span></td>
                    @endif
                    <td>
                        <div class="btn-group">
